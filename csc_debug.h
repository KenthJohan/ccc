...
#define ASSERT_LT(A,B)          do{if((A)>=(B)){assert_format(ASSERT_CARGS, NULL, "number " ASSERT_TCOLID "%s=%jd" TCOL_RST " must be less than " ASSERT_TCOLID "%s=%jd" TCOL_RST, #A, (intmax_t)(A), #B, (intmax_t)(B));}}while(0)
#define ASSERT_GT(A,B)          do{if((A)<=(B)){assert_format(ASSERT_CARGS, NULL, "number " ASSERT_TCOLID "%s=%jd" TCOL_RST " must be larger than " ASSERT_TCOLID "%s=%jd" TCOL_RST, #A, (intmax_t)(A), #B, (intmax_t)(B));}}while(0)
